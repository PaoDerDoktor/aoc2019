...
class Intcomputer(object):
    ...
    def _load(self, addr: int) -> int:
        ...
        if addr < 0:
            raise IndexError(f"RAM ACCESS ERROR : INDEX CAN'T BE NEGATIVE ({addr})")
    ...
    def _write(self, value: int, addr: int) -> None:
        ...
        if addr < 0:
            raise IndexError(f"RAM ACCESS ERROR : INDEX CAN'T BE NEGATIVE ({addr}) @ {self.ptr} == {self.ram[self.ptr]}")
        else:
            self.ram[addr] = value
    ...
    def _resolve_write_addr_arg(self, argNumber:int) -> int:
        ...
        if mode == 1: #----------------- Immediate mode
            raise ValueError(f"INVALID PARAMETER MODE : {mode} @ {self.ptr} (CAN'T WRITE TO IMMEDIATE MODE)")
        elif mode == 0: #--------------- Positional mode
            return value
        elif mode == 2: #--------------- Relative mode
    ...
    def _add(self) -> None:
        ...
        self._write(self._resolve_arg(0) + self._resolve_arg(1), self._resolve_write_addr_arg(2))
        self.ptr += 4
    ...
    def _out(self) -> None:
        ...
        if self.outputMethod == self.OUT_STDOUT:
            ...
            raise NotImplementedError(f"INVALID OUTPUT MODE : {self.outputMethod}")
        self.ptr += 2
    ...
    def _in(self) -> None:
        ...
        if self.inputMethod == self.IN_STDIN:
            ...
            if self.argModes[0] == 2:
                self._write(self._in_from_internal_list(), self._resolve_write_addr_arg(0))
            else:
                self._write(self._in_from_internal_list(), self._resolve_write_addr_arg(0))
        elif self.inputMethod == self.IN_PIPE:
            self._write(self._in_from_pipe(), self._resolve_write_addr_arg(0))
        else:
            raise NotImplementedError(f"INVALID INPUT MODE : {self.inputMethod}")
        self.ptr += 2
